...
| [016](016-lumen-importance-sampling.md) | Importance sampling + hierarchical probe refinement | 3-5 days | 2× quality/ray | partial-landed through V1. **V1** temporal octahedral direction jitter: all three trace shaders (SW Hi-Z / SDF / HW ray-query) now call `octel_direction_jittered(lid.xy, octel_jitter(frame))` instead of the fixed-centre `octel_direction`. `octel_jitter` is a 2D low-discrepancy sequence (Martin Roberts R2, a1=0.7548, a2=0.5698) indexed by `u.params.x = frame_index`; offsets are in `[-0.5, 0.5)` so each frame's 64 rays/probe sample slightly different points within their octel texels. Over the 4-frame probe EMA this is effectively free super-sampling — each octel's stored radiance converges to an integrated cone rather than a single-direction point sample. No extra ray cost; no extra bindings. Cross-platform clean. V2-V4: prev-frame-radiance-aware ray biasing (true importance sampling), variance + hierarchical refinement, close |
