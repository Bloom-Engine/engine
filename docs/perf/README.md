...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V3. **V1** per-mesh 32³ UDF bake. **V2** scene-wide 64³ SDF clipmap bake (40 m cube, 63 cm/voxel) from unified world-space triangles. **V3** SW SDF sphere-trace variant: new `SSGI_PROBE_TRACE_SDF_WGSL` compute pipeline slots between the HW ray-query and Hi-Z fallback paths. Runtime selection at dispatch — HW when available, else SDF when the clipmap is built, else Hi-Z. Hit shading in V3 is intentionally minimal (no mesh-card lookup since the merged clipmap has no per-instance identity): finite-difference SDF gradient → surface normal, analytic `albedo_gray × (sun × NdotL + sky × NdotUp)`. Measured Sponza `BLOOM_FORCE_SW_GI=1 --capture 200` → **58 fps (17.2 ms)**, within 2 fps of the HW path. Web + older-Android adapters now get Lumen off-screen bleed without HW RT. V4 follow-ups: broad-phase instance lookup at hit so SDF path can sample the 013 card radiance atlas (closes the quality gap with HW); WSRC clipmap probes for rays > 2 m; camera-following clipmap for open-world scenes |
