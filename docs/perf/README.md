...
| [013](013-lumen-surface-cache.md) | Surface Cache — Mesh Cards + per-frame card lighting | 1-2 weeks | HW bounce full quality | partial-landed — V1 single-axis albedo cards (one 128×128 slot per mesh in a 4096² atlas, dominant-axis ortho bake at model load, fallback to flat per-instance albedo when capture skipped). HW trace shader samples the atlas at the hit point via object-space AABB projection and multiplies by (NdotL × sun + NdotUp × sky) + emissive. Infrastructure confirmed functional (405 Sponza meshes captured on first frame; HW bind group sees the atlas; `inst.card_slot.w` gate branches as expected). Visual delta vs flat-albedo 007b is subtle on Sponza (one-bounce diffuse × textured albedo is low-frequency by nature). V2 follow-ups: 6-axis cards, per-frame card lighting compute pass with shadow-aware sun at card-texel granularity (the real win), normal + emissive atlases |
