...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V7. **V1** per-mesh 32³ UDF bake. **V2** scene-wide 64³ SDF clipmap from unified world-space triangles. **V3** SDF sphere-trace variant with 3-way runtime dispatch (HW > SDF > Hi-Z). **V4** textured SDF hits via broad-phase card-atlas lookup at hit_pos. **V5** camera-following clipmap with voxel-snapped origin + 10 m rebake threshold. **V6** World-Space Radiance Cache (16³ octahedral probes on a 120 m camera-following cube, shadow-aware analytic sun + sky bake) wired into the SDF trace's miss path — escaped rays now sample a directional envelope instead of writing black. **V7** closes the last two gaps: (a) HW ray-query path also samples WSRC on miss (new binding 7 on `probe_trace_hw_layout`, `hw_wsrc_sample` helper mirroring the SDF path, miss branch replaces `radiance = 0` with `hw_wsrc_sample(origin_ws + dir_ws × max_t, dir_ws)`), so HW and SDF traces now have identical miss behaviour — WSRC is a genuine path-agnostic envelope; (b) lighting-change invalidation: `maybe_invalidate_wsrc` now also clears `wsrc_built` when `light_dir`, `light_color × intensity`, `ambient × intensity`, or `light_vps[2]` differ from the snapshot taken at the last bake. Without this, a rotating sun or a cascade rebuild on a stationary camera would leave the cache frozen in the old envelope — the new check is 4 array comparisons per frame and triggers exactly one re-bake per real change. Snapshotted state fields on `Renderer`: `wsrc_last_sun_dir`, `_sun_color`, `_sky_color`, `_shadow_vp`. Capture verified (Sponza `BLOOM_FORCE_SW_GI` off → HW path active; cross-platform clean on iOS + tvOS cargo-check). V8 follow-ups: trilinear probe blend for spatial continuity at probe-cell boundaries; importance-sampled rebake cadence (cascade-specific intervals) once ticket 016 lands |
