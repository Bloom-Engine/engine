...
| [013](013-lumen-surface-cache.md) | Surface Cache — Mesh Cards + per-frame card lighting | 1-2 weeks | HW bounce full quality | landed (V2) — 6 signed-axis cards per mesh (±X ±Y ±Z) at 64×64 in a 4096² albedo atlas (4096 slots ⇒ 682 meshes at full 6-axis; Sponza's 405 fits). Per-frame card-lighting compute pass re-lights every populated slot with `albedo × (NdotL × sun + NdotUp × sky)` into a parallel `Rgba16Float` radiance atlas. HW probe trace picks the axis that faces the incoming ray and samples the pre-lit radiance texel — no hit-time shading math. Per-slot `normal_ws` buffer baked at capture (object-space axis × mesh transform). Capture is rate-limited to 40 slots/frame to stay within Metal's per-encoder render-pass budget; Sponza amortises to vsync-capped steady state within ~60 frames. Fallback to V1 flat-albedo path when `card_slot.w = 0` still present for scenes that exceed the slot allocator. Shadow-cascade-aware card lighting + emissive atlas deferred to V3. Measured `--capture 120` on Sponza: 58 fps (17.3 ms), visibly warm-toned indirect bounce from textured card content |
