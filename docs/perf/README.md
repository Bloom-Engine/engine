...
| [016](016-lumen-importance-sampling.md) | Importance sampling + hierarchical probe refinement | 3-5 days | 2× quality/ray | partial-landed through V3. **V1** temporal octahedral jitter via Martin Roberts R2 sequence. **V2** folds `probe_idx` into the jitter so adjacent probes sample decorrelated sub-texel positions. **V3** adds a prev-frame probe-history binding on each of the 3 trace pipelines (SW Hi-Z / SDF / HW) and uses the previous temporal-filtered luma for each octel to **modulate jitter magnitude**: already-bright octels (`prev_luma > ~1.0`) narrow their jitter to 30% of V1/V2's radius (exploit — lock in the peak direction), dark octels (`prev_luma ≈ 0`) keep full V1/V2 jitter (explore — broad coverage for newly-lit directions). This is a first-order importance-sampling approximation — no CDF precompute, no PDF weight tracking — but redirects sample density toward directions history says are productive. Implementation: new `prev_history: texture_3d<f32>` binding on each trace shader (bindings 11 / 9 / 10 for SW / HW / SDF), HW + SDF bg caches expanded to `[Option; 2]` per-prev_idx slot (matching the SW cache shape). Invalidated on probe-history texture resize and on TLAS rebuild. Cost: one `textureLoad` per probe-octel + trivial math. V4: variance-driven hierarchical refinement + close |
