...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V11. **V1** per-mesh 32³ UDF bake. **V2** scene-wide 64³ SDF clipmap. **V3** SDF sphere-trace 3-way dispatch. **V4** textured SDF hits. **V5** camera-following clipmap. **V6** WSRC (16³ octahedral probes, 120 m camera-follow cube, shadow-aware bake) → SDF miss path. **V7** HW miss + lighting-change invalidation. **V8** probe-space trilinear. **V9** octel bilinear (32 `textureLoad`/miss). **V10** 10×10 padded-border atlas + hardware sampler → 4 `textureSampleLevel`/miss. **V11** replaces V10's edge-extend border fill with **true octahedral silhouette wrap**. For each border texel in the 10×10 padded slab, the bake shader computes the wrapped real-octel direction and shades for THAT direction instead of cloning the nearest inside octel. The wrap rule: going "above" the top row (padded y=0) folds onto the top row with x mirrored (`real_ox = 8 - px`); below bottom row same. Left/right borders mirror y (`real_oy = 8 - py`). Corners keep V10's edge-extend (the double-fold at a corner has two valid representations and near-zero bilinear weight). Eliminates the directional discontinuity at octahedral silhouette edges where V10's bilinear blended into duplicated-inside values — rays pointing near the 4 equatorial axes of the octahedron now get smooth radiance across the wrap instead of a hard seam. Cross-platform clean (iOS + tvOS cargo-check). Sponza HW-RT capture verified. Metal drawable-stall flake continues to dominate perf measurement — logical perf unchanged from V10 (still 4 samples/miss), visual continuity at silhouettes is the deliverable. V12 follow-ups: importance-sampled rebake cadence once ticket 016 lands; lighting-change throttle so slow sun rotation doesn't re-bake every frame |
