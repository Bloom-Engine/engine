...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V14. **V1-V5** per-mesh UDF → scene clipmap → SDF trace → textured hits → camera-follow. **V6** WSRC + SDF miss. **V7** HW miss + lighting invalidation. **V8-V11** trilinear / octel bilinear / padded borders / octahedral wrap. **V12** V7 strict-equal invalidation → perceptual hysteresis. **V13** 3 stacked cascades (30 / 120 / 500 m) in one `(160, 160, 48)` atlas. **V14** adds **HW-ray-traced WSRC bake** on RT-capable adapters. Each probe-octel now fires a rayQuery of length `extent/2` from the probe centre in its octel direction. On hit, the bake shader samples the pre-lit Mesh Cards radiance atlas at the hit point (same projection math as `SSGI_PROBE_TRACE_HW_WGSL`'s hit branch) — which is itself shadow-aware (V3) + emissive (V3) + sun-lit per frame. On miss, falls back to V13's analytic sun×shadow + hemisphere sky. Net result: WSRC now carries **real one-bounce indirect** propagated from the card atlas, so when a SSGI probe ray misses scene geometry and reads WSRC, it effectively gets 2-bounce GI — V14 WSRC bake is itself a bounce on top of the V3 cards, and the SSGI sample is the third. SW adapters keep the V13 analytic-only bake unchanged. New `WSRC_BAKE_HW_WGSL` shader (workgroup 10×10 same as SW), 10-entry bind group (shadow × 4 + storage + TLAS + instance_data + card_atlas + card_samp). `wsrc_bake_hw_{pipeline, layout, bg_cache}` fields on `Renderer`, all `Option` — built only when `hw_rt_enabled`. HW bg cache invalidates on TLAS rebuild / resize alongside the HW trace cache. `bake_wsrc()` picks the HW pipeline when `hw_rt_enabled && tlas ready && instance_data ready`; otherwise falls through to the V13 SW bake cleanly. Cross-platform clean (iOS + tvOS cargo-check). Sponza HW-RT capture verified. V15: VRAM + SSIM audit, Web/Android smoke |
