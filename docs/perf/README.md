...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V4. **V1** per-mesh 32³ UDF bake. **V2** scene-wide 64³ SDF clipmap from unified world-space triangles. **V3** SDF sphere-trace variant with 3-way runtime dispatch (HW > SDF > Hi-Z). **V4** closes the quality gap: decouples the per-instance GI data buffer from the TLAS so SW-only adapters also populate it; un-gates card-slot allocation + card capture from `hw_rt_enabled` so the Mesh-Cards atlas exists on SW too; SDF shader now broad-phase-iterates `instance_data` at hit, finds the AABB containing `hit_pos`, picks the signed axis closest to the outward normal, projects onto the 013 card, and samples the pre-lit radiance atlas — textured hits instead of flat gray. Fallback to analytic sun+sky+gray when no instance AABB contains the hit (clipmap sentinel voxels, unaccounted geometry). Measured Sponza `BLOOM_FORCE_SW_GI=1 --capture 300` → **60 fps vsync (16.7 ms)**, matching the HW path exactly. Web + older-Android adapters now get Lumen-quality indirect bounce via a pure compute-shader path. V5 follow-ups: camera-following clipmap for open-world scenes; WSRC octahedral probes for rays > 2 m from scene geometry |
