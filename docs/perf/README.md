...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V8. **V1** per-mesh 32³ UDF bake. **V2** scene-wide 64³ SDF clipmap. **V3** SDF sphere-trace variant with 3-way runtime dispatch (HW > SDF > Hi-Z). **V4** textured SDF hits via broad-phase card-atlas lookup. **V5** camera-following clipmap. **V6** World-Space Radiance Cache (16³ octahedral probes on 120 m camera-follow cube) wired into SDF miss path. **V7** HW miss path reads same WSRC + lighting-change invalidation (`wsrc_last_sun_dir`, `_sun_color`, `_sky_color`, `_shadow_vp` snapshot). **V8** trilinear probe blend: both miss-path helpers (`wsrc_sample` SDF / `hw_wsrc_sample` HW) now do an 8-tap trilinear lookup across the 8 neighbouring probes in the grid instead of picking the nearest one. Conversion to probe-centre space (`pf = rel/cell - 0.5`) makes integer `pf` values land on probe centres; the fractional part gives trilinear weights. Nearest-octel for direction (octahedral bilinear requires edge-wrap handling — separate ticket). Removes the visible 7.5 m banding that was showing up as a ring around the camera when the SDF trace's ray terminals crossed cell boundaries. Factored out `wsrc_load_cell` / `hw_wsrc_load_cell` helpers (clamp + textureLoad) so the 8 cube-corner reads stay legible. Sponza HW-RT capture verified. Cross-platform clean (iOS + tvOS cargo-check). V9 follow-ups: octahedral bilinear for direction continuity; importance-sampled rebake cadence once ticket 016 lands |
