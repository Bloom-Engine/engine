...
| [017](017-lumen-cleanup-and-protocol-gaps.md) | Lumen rollout cleanup — regression guard + `--ssgi 0` hang + 016 scope match + untracked state | 1-2 days | protocol compliance | open (`--quality 0` regression guard fails at 2.0 fps vs 60 fps target; `--ssgi 0` path hits 0.1 fps on some runs; 016 closure describes CDF importance + hierarchical refinement but shipped variance-adaptive EMA instead; uncommitted `native/web/src/lib.rs` + untracked `tools/*_reference/` dirs) |
