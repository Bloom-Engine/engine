...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V13. **V1** per-mesh UDF. **V2** scene SDF clipmap. **V3** 3-way dispatch. **V4** textured SDF hits. **V5** camera-follow clipmap. **V6** WSRC + SDF miss. **V7** HW miss + lighting invalidation. **V8-V11** trilinear / octel bilinear / padded borders / octahedral wrap. **V12** V7's strict-equal invalidation → perceptual hysteresis (1° sun angle, 5 % luma delta). **V13** upgrades WSRC from a single 120 m cube to **3 stacked cascades**: near 30 m (1.875 m/cell), mid 120 m (7.5 m/cell), far 500 m (31.25 m/cell). Atlas grows from (160, 160, 16) to (160, 160, 48) — same XY layout, 3× Z depth — with each 16-slice block holding one cascade. Bake shader gains a `flags.z = cascade_idx` uniform field and offsets its output Z by `cascade_idx * 16`; `bake_wsrc()` loops the existing pipeline 3× with per-cascade origins + extents. Per-cascade `Renderer` state: `wsrc_built: [bool; 3]`, `wsrc_origin: [[f32; 3]; 3]`, `wsrc_last_{sun_dir, sun_color, sky_color}: [...; 3]`. Invalidation runs V12 hysteresis per cascade — a sun rotation invalidates all three (sun moves on every cascade equally); camera motion typically invalidates only the near cascade until the threshold scales up. `ProbeTraceParams` replaces `wsrc: vec4<f32>` with `wsrc_cascades: array<vec4<f32>, 3>`; both miss helpers (`wsrc_sample` SDF / `hw_wsrc_sample` HW) gain a `pick_cascade(pos_ws)` that returns the smallest cascade whose cube contains the ray-terminal position (unbaked cascades = skipped). Sample cost unchanged vs V10/V11 — still 4 `textureSampleLevel` per miss (inside the picked cascade). Near-range bounces now have ~4× higher spatial resolution than V10 while far-range bounces stay covered out to 500 m instead of returning black. VRAM: atlas grows from 3.3 MB to 9.8 MB (total). Cross-platform clean (iOS + tvOS cargo-check). Sponza HW-RT capture verified. V14-V15: HW-ray-traced WSRC bake, VRAM + SSIM audit |
