...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V2. **V1** bakes a per-mesh 32³ R32Float UDF via a compute pass (`SDF_BAKE_WGSL`), rate-limited to 8 meshes/frame. **V2** bakes a scene-wide 64³ R32Float SDF clipmap (40 m cube, 63 cm/voxel) once all per-mesh queues drain — CPU gathers every visible mesh's triangles into a unified world-space vertex + index buffer (3.7M triangles on Sponza), one compute dispatch runs the same bake shader against the scene-wide data. Reuses V1's compute pipeline verbatim; distinguishes per-mesh vs scene-wide solely by the bound storage buffers + AABB params. Both land silently — no trace consumer yet. Next: sphere-trace variant of the SW probe shader that marches the clipmap + reads card radiance at hit, then WSRC clipmap probes for rays that march > 2 m without hitting |
