...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V10. **V1** per-mesh 32³ UDF bake. **V2** scene-wide 64³ SDF clipmap. **V3** SDF sphere-trace variant with 3-way runtime dispatch. **V4** textured SDF hits. **V5** camera-following clipmap. **V6** World-Space Radiance Cache (16³ octahedral probes, 120 m camera-follow cube, shadow-aware bake). **V7** HW miss path reads WSRC + lighting-change invalidation. **V8** trilinear probe blend. **V9** octel bilinear (32 `textureLoad`/miss). **V10** collapses the 32 loads to **4 `textureSampleLevel`**: WSRC atlas grows from (128,128,16) to (160,160,16) with a 1-texel edge-extended border around each probe's 8×8 octel slab (padded to 10×10), and a linear Rgba16Float sampler does the bilinear-XY-within-one-probe blend natively. Z axis is unpadded — probes live on consecutive Z slices, so sampling at `(gz + 0.5 + fz) / 16` gives cross-probe Z trilinear for free. X/Y cross-probe blend is still manual (4 sample calls, one per XY corner), since the per-probe borders prevent the sampler from leaking adjacent-probe octel data across probe seams. Bake shader now workgroups `10×10×1` (vs V6's `8×8×1`), each thread computes one padded texel via `octel_direction(clamp(padded-1, 0, 7))` — border threads shade for the nearest inside octel direction. New `wsrc_atlas_sampler` field + binding 9 on the SDF layout / binding 8 on the HW layout. Sponza HW-RT capture verified. Cross-platform clean (iOS + tvOS cargo-check). Measured perf bound by Metal drawable-stall flake throughout V8-V10 testing; V10 is expected ~4× faster than V9's miss path on the analytic count (4 samples vs 32 loads), but a reliable steady-state FPS requires a fresh Metal state. V11 follow-ups: true octahedral silhouette wrap (vs V10's edge-extend borders); importance-sampled rebake cadence once ticket 016 lands |
