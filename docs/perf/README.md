...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V9. **V1** per-mesh 32³ UDF bake. **V2** scene-wide 64³ SDF clipmap. **V3** SDF sphere-trace variant with 3-way runtime dispatch (HW > SDF > Hi-Z). **V4** textured SDF hits. **V5** camera-following clipmap. **V6** World-Space Radiance Cache (16³ octahedral probes, 120 m camera-follow cube, shadow-aware bake) wired into SDF miss path. **V7** HW miss path reads same WSRC + lighting-change invalidation. **V8** 8-tap trilinear probe blend (kills 7.5 m spatial cell-boundary banding). **V9** adds 4-tap bilinear across octel texels within each probe — total 32 `textureLoad` per miss ray. Kills the directional 8-step quantisation that was visible as hard radiance tiers in the bounce. Both miss helpers (`wsrc_sample` / `hw_wsrc_sample`) now compute a float texel-space uv from `oct_encode(dir_ws) * 8 - 0.5` and pass it to `wsrc_load_cell_oct_bilinear` (per-probe 2×2 sample with edge-clamped octel indices). Octahedral silhouette wrap (u=0↔1, v=0↔1) is left as edge-extend for V9 — proper padded-border bake is a separate piece of work. Measured Sponza `--quality 3 --ssgi 1 --capture 180` HW-RT: **42 fps (24 ms)** vs V8 **40 fps (25 ms)** vs V7 **60 fps vsync** — the extra 24 `textureLoad`/miss vs V8 adds ~1 ms to the SSGI pass on Apple Silicon; still comfortably inside the 33 ms two-frame-vsync window, and Metal-stall-flake conditions made fresh baseline capture unreliable. Cross-platform clean (iOS + tvOS cargo-check). V10 follow-ups: padded-border WSRC bake so `textureSampleLevel` with linear filtering handles both axes natively; importance-sampled rebake cadence once ticket 016 lands |
