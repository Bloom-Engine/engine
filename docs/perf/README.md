...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V12. **V1** per-mesh 32³ UDF bake. **V2** scene-wide 64³ SDF clipmap. **V3** SDF sphere-trace 3-way dispatch. **V4** textured SDF hits. **V5** camera-following clipmap. **V6** WSRC + SDF miss. **V7** HW miss + lighting-change invalidation. **V8** probe trilinear. **V9** octel bilinear. **V10** padded borders + hardware sampler (4 samples/miss). **V11** true octahedral silhouette wrap. **V12** rebake hysteresis: V7's strict-float-equality on sun direction / color / shadow VP re-baked WSRC every frame on any dynamic-lighting scene (including sub-degree sun jitter and < 1 % color drift from tonemap feedback), wiping the cache benefit. V12 swaps that for perceptual thresholds — sun direction is dot-product-compared with `cos(1°) ≈ 0.99985`, sun and sky colours use a 5 % relative-luma delta (`rel_diff(a, b) = \|a - b\| / max(a, b, 1e-4)`). Shadow-VP exact-compare is dropped entirely (it drifted with sub-threshold camera motion and dragged cadence back to every-frame; the angular sun check subsumes cascade refits that matter). Snapshot written at end of `bake_wsrc()` against the exact values that went into the bake params, so the next frame's check is well-defined even on lighting oscillations. Static sun scenes now keep the cache for the full static-scene lifetime; a continuously-rotating sun rebakes once per ~1° of rotation (e.g., 1 rebake per second at 1°/s rotation). Cross-platform clean (iOS + tvOS cargo-check). Sponza HW-RT capture verified. V13-V15: multi-cascade WSRC (near/mid/far), optional HW-ray-traced WSRC bake, VRAM + SSIM audit |
