...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | foundation landed (V1) — per-mesh unsigned-distance-field bake via a compute pass. `MESH_SDF_RES = 32` voxels cubed, R32Float storage texture per mesh (128 KB × 405 meshes ≈ 52 MB on Sponza). Bake shader reads vertex + index buffers as storage, iterates all triangles per voxel, writes `min(point-triangle distance)`. Rate-limited to 8 meshes per frame. Vertex/index buffer usage widened to include STORAGE on RT-capable adapters. No trace integration yet — SDF textures are baked and sit idle until a follow-up wires sphere-marching into the SW probe trace. Next: global SDF clipmap merge pass, WSRC clipmap probes, sphere-trace variant of the probe shader |
