...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V6. **V1** per-mesh 32³ UDF bake. **V2** scene-wide 64³ SDF clipmap from unified world-space triangles. **V3** SDF sphere-trace variant with 3-way runtime dispatch (HW > SDF > Hi-Z). **V4** textured SDF hits via broad-phase card-atlas lookup at hit_pos. **V5** camera-following clipmap with voxel-snapped origin + 10 m rebake threshold. **V6** World-Space Radiance Cache: 16³ octahedral probes (8×8 per probe) on a 120 m camera-following cube (3× the SDF clipmap), baked every invalidation from analytic sun × shadow-cascade-2 + analytic sky. SDF trace's miss path now samples the WSRC atlas at the ray's terminal position along the ray direction instead of writing black — gives escaped rays a directional radiance envelope so sky/sun contribution survives through the temporal EMA. Shadow-cascade lookup at probe position means arch-occluded probes carry zero sun but still carry sky, so undercover rays get a plausible "bluish gray" fallback rather than flat sunlight everywhere; the cache is coarse-directional (8×8 octahedral) on purpose — distant envelope is low-spatial-frequency, directional resolution matters more than probe density. Shares the same camera-follow invalidation pattern as the clipmap (30 m threshold on 120 m). Miss fallback to pre-V6 black only when WSRC is not yet baked (first frame). Uniform layout: `ProbeTraceParams` gains a `wsrc: vec4<f32>` (origin xyz + extent w); SDF trace bind group gains binding 8 (non-filtering `texture_3d<f32>`, textureLoad-sampled — no sampler needed). WSRC bake pipeline is 1 dispatch of `16 × 16 × 16` workgroups @ 8×8 threads = 262144 invocations of analytic-only math + 1 shadow sample per texel. Cross-platform clean (iOS + tvOS cargo-check). V7 follow-up: wire WSRC into the HW ray-query miss path + invalidate on lighting changes (sun rotation / cascade rebuild) |
