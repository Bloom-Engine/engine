...
| [014](014-lumen-mesh-sdfs.md) | Per-mesh SDFs + global SDF clipmap + WSRC | 3-4 weeks | SW parity for Android / web | partial-landed through V5. **V1** per-mesh 32³ UDF bake. **V2** scene-wide 64³ SDF clipmap from unified world-space triangles. **V3** SDF sphere-trace variant with 3-way runtime dispatch (HW > SDF > Hi-Z). **V4** textured SDF hits: decouples the per-instance GI data buffer from the TLAS so SW-only adapters populate it; un-gates card-slot allocation + card capture from `hw_rt_enabled`; SDF shader broad-phase-iterates `instance_data` at hit, picks the signed axis closest to the outward normal, projects onto the 013 card, and samples the pre-lit radiance atlas — textured hits instead of flat gray. Fallback to analytic sun+sky+gray when no instance AABB contains the hit. **V5** camera-following clipmap: origin is the voxel-snapped camera world position (read from `current_camera_pos` which `begin_mode_3d` writes every frame), stored on the `Renderer` and piped into the SDF trace uniform in place of the removed `SCENE_SDF_CLIPMAP_ORIGIN` const. Per-frame `maybe_invalidate_sdf_clipmap` clears the `built` flag when the camera has moved more than `0.25 × 40 m = 10 m` from the current centre; the next frame's bake re-centres and the SDF trace gets a fresh 40 m window around wherever the player is. Voxel-snapping (40/64 ≈ 0.625 m) keeps sphere-trace steps deterministic across sub-voxel camera motion. Single-frame fallback to Hi-Z between invalidation and rebake completion. Measured Sponza `BLOOM_FORCE_SW_GI=1 --capture 300` → **60 fps vsync (16.7 ms)** with camera-tracking clipmap active; first bake logs `cam=(0.0, 2.0, 0.0)` at Sponza's default eye-level camera. V6 follow-up: WSRC octahedral probes for rays > 2 m from clipmap boundary |
