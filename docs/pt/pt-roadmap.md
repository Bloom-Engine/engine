...
- **Layered-material sidecar**: a 96-byte scalar record and group-2 pipeline
  ...
  unchanged. Resolved specular-factor/color, clearcoat-factor/roughness, and
  sheen-color/roughness textures use the existing texture array plus separate
  lazy 64-byte transform records, with transformed UV0/UV1 reconstruction at
  primary and bounce hits. UV1 is an aligned 8-byte-per-vertex sidecar retained
  for static and skinned geometry only when selected. White textures are
  byte-identical to the corresponding scalar path. Missing UV1 streams,
  clearcoat normal maps, and other texture-bearing lobes continue to use exact
