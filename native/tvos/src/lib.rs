...
unsafe extern "C" fn scene_will_connect(
    ...
    eprintln!("[bloom-tvos] renderer device negotiation = {negotiation_report}");
...
fn request_renderer_device(
    ...
    .unwrap_or_else(|error| panic!("[bloom-tvos] Failed to create renderer device: {error}"))
}
...
/// host owns; `width`/`height` are its size in points. Returns 1.0 on
/// success, 0.0 on a null/invalid handle or surface bring-up failure.
/// Idempotent once attached. tvOS uses a non-sRGB swapchain to match its
/// windowed path.
#[no_mangle]
    ...
    match unsafe {
        bloom_shared::attach::attach_engine(
            ...
            bloom_shared::attach::AttachParams {
                ...
                format: bloom_shared::attach::FormatPreference::NonSrgb,
            },
        )
    } {
        Ok(es) => {
