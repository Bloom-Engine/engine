// Transient resource pool — RFC 0001 Phase 3 + issue #129.
//
...
//
// The compiled path consumes graph lifetime/physical-slot assignments through
// `prepare_compiled_plan`. Logical resources assigned to one conservative
// slot share one wgpu texture allocation; persistent/history imports never
// enter this pool. The legacy acquire/release API remains for older internal
// consumers and its focused tests, but the live scene snapshot path uses only
// compiled allocations.
//
// Deferred:
//
...
impl TransientDesc {
    pub fn new(format: wgpu::TextureFormat, usage: wgpu::TextureUsages, size: SizePolicy) -> Self {
        Self {
            format,
            usage,
            size,
...
pub struct TransientPool {
    ...
    /// #129 — physical allocations owned by an immutable compiled lifetime
    /// plan. Exact extents form a separate resize generation.
    compiled_plans: HashMap<u64, CompiledPlan>,
    compiled_extent: ((u32, u32), (u32, u32)),
}
...
impl TransientPool {
    ...
    pub fn invalidate_swapchain_relative(&mut self) {
        ...
        self.slots
            .retain(|s| matches!(s.desc.size, SizePolicy::Fixed(_, _)));
        let had_compiled = !self.compiled_plans.is_empty();
        self.compiled_plans.clear();
        self.compiled_extent = ((0, 0), (0, 0));
        if self.slots.len() != before || had_compiled {
    ...
    pub fn clear(&mut self) {
        if !self.slots.is_empty() || !self.compiled_plans.is_empty() {
            self.slots.clear();
            self.compiled_plans.clear();
            self.compiled_extent = ((0, 0), (0, 0));
            self.rebuild_epoch += 1;
    ...
    /// Materialize the texture allocations selected by a compiled graph.
    /// Repeated calls for a stable plan/extent are allocation-free.
    pub fn prepare_compiled_plan(
        &mut self,
        device: &wgpu::Device,
        ...
        if self.compiled_extent != extent_key {
            if self.compiled_extent != ((0, 0), (0, 0)) && !self.compiled_plans.is_empty() {
                self.rebuild_epoch = self.rebuild_epoch.saturating_add(1);
            }
            self.compiled_plans.clear();
        ...
        for allocation in &plan.allocations {
            let label = format!(
                ...
                plan.plan_id, allocation.id.0
            );
            let slot = match &allocation.desc {
                graph::ResourceDesc::Texture(desc) => {
                    let (width, height, depth_or_array_layers) =
                        desc.extent.resolve(render_extent, output_extent);
                    let dimension = match desc.dimension {
                        graph::TextureDimension::D1 => wgpu::TextureDimension::D1,
                        graph::TextureDimension::D2 => wgpu::TextureDimension::D2,
                        graph::TextureDimension::D3 => wgpu::TextureDimension::D3,
                    };
                    let usage = compiled_texture_usage(desc.allowed_usage);
            ...
            slots.push(slot);
            physical_to_slot.insert(allocation.id, slot_index);
        }

        let mut resource_to_slot = HashMap::new();
        for resource in &plan.resources {
            let Some(physical) = resource.physical else {
                continue;
            };
            let slot = physical_to_slot
                .get(&physical)
                .copied()
                .ok_or_else(|| format!("missing physical allocation {}", physical.0))?;
            resource_to_slot.insert(resource.id, slot);
        }
    ...
    pub fn compiled_texture(
        ...
        let plan = self.compiled_plans.get(&plan_id)?;
        let slot = *plan.resource_to_slot.get(&resource)?;
        match plan.slots.get(slot)? {
            CompiledSlot::Texture { texture, .. } => Some(texture),
            CompiledSlot::Buffer(_) => None,
        }
    }

    pub fn compiled_view(
        &self,
        plan_id: u64,
        resource: graph::ResourceId,
        ...
        let slot = *plan.resource_to_slot.get(&resource)?;
        match plan.slots.get(slot)? {
            CompiledSlot::Texture { view, .. } => Some(view),
            CompiledSlot::Buffer(_) => None,
        }
    }

    pub fn compiled_buffer(
        &self,
        plan_id: u64,
        resource: graph::ResourceId,
    ) -> Option<&wgpu::Buffer> {
        ...
        let slot = *plan.resource_to_slot.get(&resource)?;
        match plan.slots.get(slot)? {
            CompiledSlot::Buffer(buffer) => Some(buffer),
            CompiledSlot::Texture { .. } => None,
        }
    }

    pub fn compiled_slot_count(&self) -> usize {
        self.compiled_plans
            .values()
            .map(|plan| plan.slots.len())
            .sum()
    }
    ...
    pub fn acquire(&mut self, device: &wgpu::Device, desc: TransientDesc) -> TransientId {
        ...
        for slot in self.slots.iter_mut() {
            if !slot.in_use && slot.desc == desc && slot.extent == target_extent {
                slot.in_use = true;
...
fn compiled_texture_usage(usage: graph::TextureUsage) -> wgpu::TextureUsages {
    let mut result = wgpu::TextureUsages::empty();
    for (declared, backend) in [
        (graph::TextureUsage::COPY_SRC, wgpu::TextureUsages::COPY_SRC),
        (graph::TextureUsage::COPY_DST, wgpu::TextureUsages::COPY_DST),
        (
            graph::TextureUsage::SAMPLED,
            wgpu::TextureUsages::TEXTURE_BINDING,
        ),
        (
            graph::TextureUsage::STORAGE_READ,
            wgpu::TextureUsages::STORAGE_BINDING,
        ),
        (
            graph::TextureUsage::STORAGE_WRITE,
            wgpu::TextureUsages::STORAGE_BINDING,
        ),
        (
            graph::TextureUsage::COLOR_ATTACHMENT,
            wgpu::TextureUsages::RENDER_ATTACHMENT,
        ),
        (
            graph::TextureUsage::DEPTH_ATTACHMENT_READ,
            wgpu::TextureUsages::RENDER_ATTACHMENT,
        ),
        (
            graph::TextureUsage::DEPTH_ATTACHMENT_WRITE,
            wgpu::TextureUsages::RENDER_ATTACHMENT,
        ),
    ] {
        if usage.contains(declared) {
            result |= backend;
        }
    }
    result
...
fn compiled_buffer_usage(usage: graph::BufferUsage) -> wgpu::BufferUsages {
    let mut result = wgpu::BufferUsages::empty();
    for (declared, backend) in [
        (graph::BufferUsage::COPY_SRC, wgpu::BufferUsages::COPY_SRC),
        (graph::BufferUsage::COPY_DST, wgpu::BufferUsages::COPY_DST),
        (graph::BufferUsage::UNIFORM, wgpu::BufferUsages::UNIFORM),
        (
            graph::BufferUsage::STORAGE_READ,
            wgpu::BufferUsages::STORAGE,
        ),
        (
            graph::BufferUsage::STORAGE_WRITE,
            wgpu::BufferUsages::STORAGE,
        ),
        (graph::BufferUsage::VERTEX, wgpu::BufferUsages::VERTEX),
        (graph::BufferUsage::INDEX, wgpu::BufferUsages::INDEX),
        (graph::BufferUsage::INDIRECT, wgpu::BufferUsages::INDIRECT),
    ] {
        if usage.contains(declared) {
            result |= backend;
        }
...
mod tests {
    ...
    fn size_policy_resolves() {
        ...
        assert_eq!(
            SizePolicy::Fixed(2048, 2048).extent(1920, 1080),
            (2048, 2048)
        );
    }
    ...
    fn resize_drops_swapchain_relative_slots() {
        ...
        assert_ne!(
            a.0, a2.0,
            "post-resize slot ids should not collide with released ones"
        );
    }
    ...
    #[test]
    fn compiled_plan_allocates_once_per_plan_and_resize_generation() {
        let Some((device, _queue)) = try_create_device() else {
            return;
        };
        let key = graph::FramePlanKey {
            resolution: graph::ResolutionClass::Medium,
            quality_tier: 3,
            feature_mask: graph::FRAME_FEATURE_SCENE_SNAPSHOTS,
            capability: graph::CapabilityTier::Raster,
            path_tracing: graph::PathTracingMode::Off,
            post_pass_count: 0,
            render_target_output: false,
        };
        let plan = graph::build_renderer_frame_plan(key, wgpu::TextureFormat::Bgra8UnormSrgb)
            .compile(graph::CompileOptions::CONSERVATIVE_ALIASING)
            .unwrap();
        let color = plan.resource("translucent-scene-color").unwrap().id;
    ...
    #[test]
    fn compiled_plan_materializes_aliased_buffer_slots() {
        let Some((device, _queue)) = try_create_device() else {
            return;
        };
        let usage = graph::BufferUsage::COPY_DST.union(graph::BufferUsage::STORAGE_READ);
        let desc = graph::BufferDesc {
            size: 1024,
            allowed_usage: usage,
            alias_class: graph::AliasClass::Storage,
        };
        let mut builder = graph::GraphBuilder::new("buffer-materialization");
        let a = builder.create_buffer("a", desc.clone());
        let b = builder.create_buffer("b", desc);
        let write_a = builder.add_pass("write-a");
        let a = builder.write_buffer(write_a, a, graph::BufferUsage::COPY_DST);
        let read_a = builder.add_pass("read-a");
        builder.after(read_a, write_a);
        builder.read_buffer(read_a, a, graph::BufferUsage::STORAGE_READ);
        let write_b = builder.add_pass("write-b");
        builder.after(write_b, read_a);
        let b = builder.write_buffer(write_b, b, graph::BufferUsage::COPY_DST);
        let read_b = builder.add_pass("read-b");
        builder.after(read_b, write_b);
        builder.read_buffer(read_b, b, graph::BufferUsage::STORAGE_READ);
        let plan = builder
            .compile(graph::CompileOptions::CONSERVATIVE_ALIASING)
            .unwrap();
        ...
        assert!(pool
            ...
            .is_some());
        assert!(pool
            .compiled_buffer(plan.plan_id, plan.resource("b").unwrap().id)
            .is_some());
    }
}
