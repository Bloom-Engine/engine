...
impl GpuDrivenRenderer {
    pub fn new(
        ...
        let indirect_buffer = create_indirect_buffer(device, draw_capacity);
        ...
        let cull_layout = create_cull_layout(device);
    ...
    fn ensure_draw_capacity(&mut self, device: &wgpu::Device, required: usize) {
        ...
        self.indirect_buffer = create_indirect_buffer(device, self.draw_capacity);
...
fn create_indirect_buffer(device: &wgpu::Device, capacity: usize) -> wgpu::Buffer {
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("gpu_driven_indirect_commands"),
