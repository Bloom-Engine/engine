...
fn cs_main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let geometry_valid = probe_history_geometry_valid(probes[probe_idx]);
    ...
    if (!valid_probe || u.params.y > 0.5) {
        ...
        // Cyan distinguishes a moved/disoccluded surface from radiance-only
        // adaptation, making the world-space continuity guard testable.
        reason = vec3<f32>(0.0, 0.8, 1.0);
