...
impl Renderer {
    fn ensure_scene_iridescence_ssr_resources(&mut self) {
        ...
        let metadata_view =
            metadata_texture.create_view(&wgpu::TextureViewDescriptor::default());
    ...
    fn append_cached_iridescence_draws<'a>(
        ...
        for command in &self.model_draw_commands {
            ...
            let (mesh_draw, vertex_byte_offset, index_byte_offset) =
                if secondary_uv.is_some() {
                    self.gpu_driven
