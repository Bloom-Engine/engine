...
impl JoltPhysics {
    ...
    pub fn scratch_push_f32(&mut self, v: f32) {
        self.scratch_f32.push(v);
    }
    pub fn scratch_push_u32(&mut self, v: u32) {
    ...
    pub fn shape_convex_hull_from_scratch(&mut self, num_points: u32, convex_radius: f32) -> f64 {
        ...
        if self.scratch_f32.len() < need || num_points < 3 {
            return 0.0;
        }
        let points: Vec<BjVec3> = (0..num_points as usize)
            .map(|i| BjVec3 {
    ...
    pub fn shape_mesh_from_scratch(&mut self, vertex_count: u32, triangle_count: u32) -> f64 {
        ...
        let vertices: Vec<BjVec3> = (0..vertex_count as usize)
            .map(|i| BjVec3 {
                x: self.scratch_f32[i * 3],
                y: self.scratch_f32[i * 3 + 1],
                z: self.scratch_f32[i * 3 + 2],
    ...
    pub fn shape_heightfield_from_scratch(
        ...
        if self.scratch_f32.len() < need || sample_count < 2 {
            return 0.0;
        }
    ...
    pub fn set_gravity(&self, world_h: f64, x: f32, y: f32, z: f32) {
        if let Some(&world) = self.worlds.get(world_h) {
            unsafe {
                bj_world_set_gravity(world, BjVec3 { x, y, z });
            }
        }
    }
    ...
    pub fn step(&mut self, world_h: f64, dt: f32, collision_steps: u32) {
        if let Some(&world) = self.worlds.get(world_h) {
            unsafe {
                bj_world_step(world, dt, collision_steps.max(1));
            }
    ...
    fn drain_contacts(&mut self, world: bj_world) {
        ...
        if count > 0 {
            self.contact_cache
                .events
                .resize(count as usize, unsafe { std::mem::zeroed() });
            let drained = unsafe {
    ...
    pub fn step_fixed(&mut self, world_h: f64, frame_dt: f32, collision_steps: u32) -> f32 {
        let Some(&world) = self.worlds.get(world_h) else {
            return 1.0;
        };
        let dt = if frame_dt.is_finite() {
            frame_dt.clamp(0.0, MAX_FRAME_DT)
        } else {
            0.0
        };
        ...
        if steps > 0 {
            ...
            for i in 0..steps {
                ...
                unsafe {
                    bj_world_step(world, fixed, collision_steps.max(1));
                }
            }
            if let Some(s) = snap {
    ...
    fn snapshot_world_bodies(
        &self,
        world: bj_world,
    ) -> std::collections::HashMap<u64, (BjVec3, BjQuat)> {
        self.bodies
            .iter()
            .filter(|(_, &(w, _))| w == world)
            .map(|(_, &(w, b))| {
    ...
    pub fn set_layer_collides(&self, world_h: f64, a: u32, b: u32, collides: bool) {
        if let Some(&world) = self.worlds.get(world_h) {
            unsafe {
                bj_world_set_layer_collides(world, a, b, collides as u8);
            }
        }
    }
    ...
    pub fn create_convex_hull_shape(&mut self, points: &[BjVec3], convex_radius: f32) -> f64 {
        let s =
            unsafe { bj_shape_convex_hull(points.as_ptr(), points.len() as u32, convex_radius) };
        if s == BJ_INVALID {
            0.0
        } else {
            self.shapes.alloc(s)
    ...
    pub fn create_heightfield_shape(
        ...
        let s = unsafe {
            bj_shape_heightfield(samples.as_ptr(), sample_count, offset, scale, block_size)
        };
        if s == BJ_INVALID {
            0.0
        } else {
    ...
    pub fn create_scaled_shape(&mut self, base_h: f64, sx: f32, sy: f32, sz: f32) -> f64 {
        let base = match self.shapes.get(base_h) {
            Some(&s) => s,
            None => return 0.0,
        };
        let s = unsafe {
    ...
    pub fn shape_bounds_axis(&self, handle: f64, axis: u32) -> f64 {
        if let Some(&s) = self.shapes.get(handle) {
            ...
            unsafe {
                bj_shape_get_local_bounds(s, &mut mn, &mut mx);
            }
            return match axis {
                0 => mn.x,
                1 => mn.y,
    ...
    pub fn shape_volume(&self, handle: f64) -> f32 {
        self.shapes
            .get(handle)
            .map(|&s| unsafe { bj_shape_get_volume(s) })
            .unwrap_or(0.0)
    }
    ...
    pub fn create_body(
        ...
        let world = match self.worlds.get(world_h) {
            Some(&w) => w,
            None => return 0.0,
        };
        let shape = match self.shapes.get(shape_h) {
            Some(&s) => s,
            None => return 0.0,
        };
        ...
        let desc = BjBodyDesc {
            ...
            linear_velocity: BjVec3 {
                x: vx,
                y: vy,
                z: vz,
            },
            angular_velocity: BjVec3 {
                x: wx,
                y: wy,
                z: wz,
            },
    ...
    pub fn body_deactivate(&self, h: f64) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_deactivate(w, b);
            }
        }
    }
    pub fn body_is_active(&self, h: f64) -> bool {
        self.resolve_body(h)
            .map(|(w, b)| unsafe { bj_body_is_active(w, b) } != 0)
            .unwrap_or(false)
    }
    pub fn body_is_valid(&self, h: f64) -> bool {
        self.resolve_body(h)
            .map(|(w, b)| unsafe { bj_body_is_valid(w, b) } != 0)
            .unwrap_or(false)
    }
    ...
    pub fn body_set_position(&self, h: f64, x: f32, y: f32, z: f32, activate: bool) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_position(
                    w,
                    b,
                    BjVec3 { x, y, z },
                    if activate {
                        BjActivation::Activate
                    } else {
                        BjActivation::DontActivate
                    },
                );
            }
        }
    ...
    pub fn body_move_kinematic(
        ...
        if let Some((w, b)) = self.resolve_body(h) {
            ...
            unsafe {
                bj_body_move_kinematic(w, b, &target, dt);
            }
        }
    }
    ...
    pub fn body_get_linear_velocity_axis(&self, h: f64, axis: u32) -> f64 {
        if let Some((w, b)) = self.resolve_body(h) {
            let mut v = BjVec3::default();
            unsafe {
                bj_body_get_linear_velocity(w, b, &mut v);
            }
            return match axis {
                0 => v.x,
                1 => v.y,
    ...
    pub fn body_get_angular_velocity_axis(&self, h: f64, axis: u32) -> f64 {
        if let Some((w, b)) = self.resolve_body(h) {
            let mut v = BjVec3::default();
            unsafe {
                bj_body_get_angular_velocity(w, b, &mut v);
            }
            return match axis {
                0 => v.x,
                1 => v.y,
    ...
    pub fn body_get_point_velocity_axis(
        &self,
        h: f64,
        px: f32,
        py: f32,
    ...
    pub fn body_set_linear_velocity(&self, h: f64, x: f32, y: f32, z: f32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_linear_velocity(w, b, BjVec3 { x, y, z });
            }
        }
    }
    pub fn body_set_angular_velocity(&self, h: f64, x: f32, y: f32, z: f32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_angular_velocity(w, b, BjVec3 { x, y, z });
            }
        }
    }
    ...
    pub fn body_add_force(&self, h: f64, x: f32, y: f32, z: f32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_add_force(w, b, BjVec3 { x, y, z });
            }
        }
    }
    pub fn body_add_impulse(&self, h: f64, x: f32, y: f32, z: f32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_add_impulse(w, b, BjVec3 { x, y, z });
            }
        }
    }
    pub fn body_add_torque(&self, h: f64, x: f32, y: f32, z: f32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_add_torque(w, b, BjVec3 { x, y, z });
            }
        }
    }
    pub fn body_add_angular_impulse(&self, h: f64, x: f32, y: f32, z: f32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_add_angular_impulse(w, b, BjVec3 { x, y, z });
            }
        }
    }
    #[allow(clippy::too_many_arguments)]
    ...
    pub fn body_set_friction(&self, h: f64, v: f32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_friction(w, b, v);
            }
        }
    }
    pub fn body_set_restitution(&self, h: f64, v: f32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_restitution(w, b, v);
            }
        }
    }
    pub fn body_set_linear_damping(&self, h: f64, v: f32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_linear_damping(w, b, v);
            }
        }
    }
    pub fn body_set_angular_damping(&self, h: f64, v: f32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_angular_damping(w, b, v);
            }
        }
    }
    pub fn body_set_gravity_factor(&self, h: f64, v: f32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_gravity_factor(w, b, v);
            }
        }
    }
    pub fn body_set_ccd(&self, h: f64, enabled: bool) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_ccd(w, b, enabled as u8);
            }
        }
    }
    ...
    pub fn body_set_object_layer(&self, h: f64, layer: u32) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_object_layer(w, b, layer);
            }
        }
    }
    pub fn body_set_is_sensor(&self, h: f64, enabled: bool) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_is_sensor(w, b, enabled as u8);
            }
        }
    }
    pub fn body_set_allow_sleeping(&self, h: f64, enabled: bool) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_allow_sleeping(w, b, enabled as u8);
            }
        }
    }
    ...
    pub fn body_lock_rotation_axes(&self, h: f64, x: bool, y: bool, z: bool) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_lock_rotation_axes(w, b, x as u8, y as u8, z as u8);
            }
        }
    }
    pub fn body_lock_translation_axes(&self, h: f64, x: bool, y: bool, z: bool) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_lock_translation_axes(w, b, x as u8, y as u8, z as u8);
            }
        }
    }

    pub fn body_get_mass(&self, h: f64) -> f32 {
        self.resolve_body(h)
            .map(|(w, b)| unsafe { bj_body_get_mass(w, b) })
            .unwrap_or(0.0)
    }
    pub fn body_get_friction(&self, h: f64) -> f32 {
        self.resolve_body(h)
            .map(|(w, b)| unsafe { bj_body_get_friction(w, b) })
            .unwrap_or(0.0)
    }
    pub fn body_get_restitution(&self, h: f64) -> f32 {
        self.resolve_body(h)
            .map(|(w, b)| unsafe { bj_body_get_restitution(w, b) })
            .unwrap_or(0.0)
    }
    pub fn body_get_object_layer(&self, h: f64) -> u32 {
        self.resolve_body(h)
            .map(|(w, b)| unsafe { bj_body_get_object_layer(w, b) })
            .unwrap_or(0)
    }
    ...
    pub fn body_set_user_data(&self, h: f64, user_data: u64) {
        if let Some((w, b)) = self.resolve_body(h) {
            unsafe {
                bj_body_set_user_data(w, b, user_data);
            }
        }
    }
    pub fn body_get_user_data(&self, h: f64) -> u64 {
        self.resolve_body(h)
            .map(|(w, b)| unsafe { bj_body_get_user_data(w, b) })
            .unwrap_or(0)
    }
    ...
    pub fn raycast_all(
        ...
        self.ray_hit_cache
            .hits
            .resize(max_hits as usize, unsafe { std::mem::zeroed() });
        let n = unsafe {
            bj_query_raycast_all(
                world,
                BjVec3 {
                    x: ox,
                    y: oy,
                ...
                max_distance,
                layer_mask,
                self.ray_hit_cache.hits.as_mut_ptr(),
                max_hits,
            )
        };
    ...
    pub fn ray_hit_count(&self) -> u32 {
        self.ray_hit_cache.hits.len() as u32
    }
    ...
    pub fn ray_hit_axis(&self, i: usize, field: u32) -> f64 {
        let h = match self.ray_hit_cache.hits.get(i) {
            Some(v) => v,
            None => return 0.0,
        };
        (match field {
    ...
    pub fn overlap_sphere(
        ...
        self.overlap_cache
            .bodies
            .resize(max_results as usize, BJ_INVALID);
        let n = unsafe {
            bj_query_overlap_sphere(
                world,
                BjVec3 {
                    x: cx,
                    y: cy,
                ...
                r,
                layer_mask,
                self.overlap_cache.bodies.as_mut_ptr(),
                max_results,
            )
        };
    ...
    fn make_anchors(
        ...
        bz: f32,
        world_space: bool,
    ) -> Option<(bj_world, BjConstraintAnchors)> {
    ...
    pub fn constraint_fixed(
        ...
        let (w, anchors) =
            match self.make_anchors(body_a, body_b, ax, ay, az, bx, by, bz, world_space) {
                Some(v) => v,
                None => return 0.0,
            };
        ...
        if c == BJ_INVALID {
            ...
            self.constraints.alloc((w, c))
        }
    }

    #[allow(clippy::too_many_arguments)]
    pub fn constraint_point(
        &mut self,
        body_a: f64,
        body_b: f64,
        ax: f32,
    ...
    pub fn constraint_six_dof_locked_translation(
        ...
        let c = unsafe { bj_constraint_six_dof(w, &anchors, trans.as_ptr(), rot_limits.as_ptr()) };
        if c == BJ_INVALID {
            0.0
        } else {
            self.constraints.alloc((w, c))
    ...
    pub fn constraint_set_enabled(&self, handle: f64, enabled: bool) {
        if let Some(&(w, c)) = self.constraints.get(handle) {
            unsafe {
                bj_constraint_set_enabled(w, c, enabled as u8);
            }
        }
    }
    ...
    pub fn contact_count(&self) -> u32 {
        self.contact_cache.events.len() as u32
    }
    ...
    pub fn contact_field(&self, i: usize, field: u32) -> f64 {
        let c = match self.contact_cache.events.get(i) {
            Some(v) => v,
            None => return 0.0,
        };
        ...
        match field {
            ...
            3 => c.point_a.x as f64,
            4 => c.point_a.y as f64,
            5 => c.point_a.z as f64,
            6 => c.point_b.x as f64,
            7 => c.point_b.y as f64,
            8 => c.point_b.z as f64,
            9 => c.normal.x as f64,
            10 => c.normal.y as f64,
            11 => c.normal.z as f64,
    ...
    pub fn character_get_position_axis(&self, h: f64, axis: u32) -> f64 {
        if let Some((w, c)) = self.resolve_character(h) {
            let mut v = BjVec3::default();
            unsafe {
                bj_character_get_position(w, c, &mut v);
            }
            return match axis {
                0 => v.x,
                1 => v.y,
    ...
    pub fn character_get_rotation_axis(&self, h: f64, axis: u32) -> f64 {
        if let Some((w, c)) = self.resolve_character(h) {
            let mut q = BjQuat::default();
            unsafe {
                bj_character_get_rotation(w, c, &mut q);
            }
            return match axis {
                0 => q.x,
                1 => q.y,
    ...
    pub fn character_set_position(&self, h: f64, x: f32, y: f32, z: f32) {
        if let Some((w, c)) = self.resolve_character(h) {
            unsafe {
                bj_character_set_position(w, c, BjVec3 { x, y, z });
            }
        }
    }
    pub fn character_set_rotation(&self, h: f64, x: f32, y: f32, z: f32, ww: f32) {
        if let Some((w, c)) = self.resolve_character(h) {
            unsafe {
                bj_character_set_rotation(w, c, BjQuat { x, y, z, w: ww });
            }
        }
    }
    pub fn character_get_linear_velocity_axis(&self, h: f64, axis: u32) -> f64 {
        if let Some((w, c)) = self.resolve_character(h) {
            let mut v = BjVec3::default();
            unsafe {
                bj_character_get_linear_velocity(w, c, &mut v);
            }
            return match axis {
                0 => v.x,
                1 => v.y,
    ...
    pub fn character_set_linear_velocity(&self, h: f64, x: f32, y: f32, z: f32) {
        if let Some((w, c)) = self.resolve_character(h) {
            unsafe {
                bj_character_set_linear_velocity(w, c, BjVec3 { x, y, z });
            }
        }
    }
    ...
    pub fn character_get_ground_normal_axis(&self, h: f64, axis: u32) -> f64 {
        if let Some((w, c)) = self.resolve_character(h) {
            let mut v = BjVec3::default();
            unsafe {
                bj_character_get_ground_normal(w, c, &mut v);
            }
            return match axis {
                0 => v.x,
                1 => v.y,
    ...
    pub fn character_get_ground_position_axis(&self, h: f64, axis: u32) -> f64 {
        if let Some((w, c)) = self.resolve_character(h) {
            let mut v = BjVec3::default();
            unsafe {
                bj_character_get_ground_position(w, c, &mut v);
            }
            return match axis {
                0 => v.x,
                1 => v.y,
    ...
    pub fn soft_body_create_from_scratch(
        ...
        if vertex_count < 3 || triangle_count == 0 {
            return 0.0;
        }
        let world = match self.worlds.get(world_h) {
            Some(&w) => w,
    ...
    pub fn soft_body_get_vertex_axis(&self, body_h: f64, idx: u32, axis: u32) -> f64 {
        if let Some((w, b)) = self.resolve_body(body_h) {
            let mut v = BjVec3::default();
            unsafe {
                bj_soft_body_get_vertex(w, b, idx, &mut v);
            }
            return match axis {
                0 => v.x,
                1 => v.y,
    ...
    pub fn soft_body_set_vertex(&self, body_h: f64, idx: u32, x: f32, y: f32, z: f32) {
        if let Some((w, b)) = self.resolve_body(body_h) {
            unsafe {
                bj_soft_body_set_vertex(w, b, idx, BjVec3 { x, y, z });
            }
        }
    }
    pub fn soft_body_set_vertex_inv_mass(&self, body_h: f64, idx: u32, inv_mass: f32) {
        if let Some((w, b)) = self.resolve_body(body_h) {
            unsafe {
                bj_soft_body_set_vertex_inv_mass(w, b, idx, inv_mass);
            }
        }
    }
    ...
    pub fn vehicle_create(
        ...
        let shape = match self.shapes.get(chassis_shape_h) {
            Some(&s) => s,
            None => return (0.0, 0.0),
        };
    ...
    pub fn vehicle_set_input(
        ...
        if let Some((w, v)) = self.resolve_vehicle(handle) {
            unsafe {
                bj_vehicle_set_input(w, v, forward, right, brake, handbrake);
            }
        }
    }
...
macro_rules! define_physics_ffi {
    () => {
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_create_world(
            gx: f64,
            gy: f64,
            gz: f64,
            max_bodies: f64,
            ...
            bloom_jolt_ffi_physics().create_world(
                gx as f32,
                gy as f32,
                gz as f32,
                max_bodies as u32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_destroy_world(world: f64) {
            bloom_jolt_ffi_physics().destroy_world(world);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_set_gravity(world: f64, gx: f64, gy: f64, gz: f64) {
            bloom_jolt_ffi_physics().set_gravity(world, gx as f32, gy as f32, gz as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_get_gravity(world: f64, axis: f64) -> f64 {
            bloom_jolt_ffi_physics().get_gravity_axis(world, axis as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_optimize_broadphase(world: f64) {
            bloom_jolt_ffi_physics().optimize_broadphase(world);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_step(world: f64, dt: f64, collision_steps: f64) {
            bloom_jolt_ffi_physics().step(world, dt as f32, collision_steps as u32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_step_fixed(
            world: f64,
            dt: f64,
            collision_steps: f64,
        ) -> f64 {
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_set_fixed_timestep(world: f64, hz: f64, max_steps: f64) {
            bloom_jolt_ffi_physics().set_fixed_timestep(world, hz as f32, max_steps as u32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_set_interpolation(world: f64, on: f64) {
            bloom_jolt_ffi_physics().set_interpolation(world, on != 0.0);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_get_step_alpha(world: f64) -> f64 {
            bloom_jolt_ffi_physics().step_alpha(world) as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_set_layer_collides(
            world: f64,
            a: f64,
            b: f64,
            collides: f64,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_get_layer_collides(world: f64, a: f64, b: f64) -> f64 {
            if bloom_jolt_ffi_physics().get_layer_collides(world, a as u32, b as u32) {
                1.0
            } else {
                0.0
            }
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_count(world: f64) -> f64 {
            bloom_jolt_ffi_physics().body_count(world) as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_active_body_count(world: f64) -> f64 {
            bloom_jolt_ffi_physics().active_body_count(world) as f64
        }

        // --- Shapes ---

        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_box(
            hx: f64,
            hy: f64,
            hz: f64,
            convex_radius: f64,
        ) -> f64 {
            bloom_jolt_ffi_physics().create_box_shape(
                hx as f32,
                hy as f32,
                hz as f32,
                convex_radius as f32,
            )
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_sphere(r: f64) -> f64 {
            bloom_jolt_ffi_physics().create_sphere_shape(r as f32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_capsule(h: f64, r: f64) -> f64 {
            bloom_jolt_ffi_physics().create_capsule_shape(h as f32, r as f32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_cylinder(h: f64, r: f64, cr: f64) -> f64 {
            bloom_jolt_ffi_physics().create_cylinder_shape(h as f32, r as f32, cr as f32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_scaled(base: f64, sx: f64, sy: f64, sz: f64) -> f64 {
            bloom_jolt_ffi_physics().create_scaled_shape(base, sx as f32, sy as f32, sz as f32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_offset_com(
            base: f64,
            ox: f64,
            oy: f64,
            oz: f64,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_release(shape: f64) {
            bloom_jolt_ffi_physics().release_shape(shape);
        }

        // Scratch streams for variable-size shape inputs.
        #[no_mangle]
        pub extern "C" fn bloom_physics_scratch_reset() {
            bloom_jolt_ffi_physics().scratch_reset();
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_scratch_push_f32(v: f64) {
            bloom_jolt_ffi_physics().scratch_push_f32(v as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_scratch_push_u32(v: f64) {
            bloom_jolt_ffi_physics().scratch_push_u32(v as u32);
        }

        // Complex shape factories — consume scratch streams populated by the caller.
        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_convex_hull(
            num_points: f64,
            convex_radius: f64,
        ) -> f64 {
            bloom_jolt_ffi_physics()
                .shape_convex_hull_from_scratch(num_points as u32, convex_radius as f32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_mesh(vertex_count: f64, triangle_count: f64) -> f64 {
            bloom_jolt_ffi_physics()
                .shape_mesh_from_scratch(vertex_count as u32, triangle_count as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_heightfield(
            sample_count: f64,
            ox: f64,
            oy: f64,
            oz: f64,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_compound_begin() {
            bloom_jolt_ffi_physics().compound_begin();
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_compound_add_child(
            shape: f64,
            px: f64,
            py: f64,
            pz: f64,
            ...
            bloom_jolt_ffi_physics().compound_add_child(
                shape, px as f32, py as f32, pz as f32, rx as f32, ry as f32, rz as f32, rw as f32,
            );
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_compound_end() -> f64 {
            bloom_jolt_ffi_physics().compound_end()
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_bounds(shape: f64, axis: f64) -> f64 {
            bloom_jolt_ffi_physics().shape_bounds_axis(shape, axis as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_shape_volume(shape: f64) -> f64 {
            bloom_jolt_ffi_physics().shape_volume(shape) as f64
        }

        // --- Bodies ---

        #[no_mangle]
        pub extern "C" fn bloom_physics_body_create(
            world: f64,
            shape: f64,
            motion_type: f64,
            px: f64,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_destroy(body: f64) {
            bloom_jolt_ffi_physics().destroy_body(body);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_activate(body: f64) {
            bloom_jolt_ffi_physics().body_activate(body);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_deactivate(body: f64) {
            bloom_jolt_ffi_physics().body_deactivate(body);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_is_active(body: f64) -> f64 {
            if bloom_jolt_ffi_physics().body_is_active(body) {
                1.0
            } else {
                0.0
            }
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_is_valid(body: f64) -> f64 {
            if bloom_jolt_ffi_physics().body_is_valid(body) {
                1.0
            } else {
                0.0
            }
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_get_position(body: f64, axis: f64) -> f64 {
            bloom_jolt_ffi_physics().body_get_position_axis(body, axis as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_get_rotation(body: f64, axis: f64) -> f64 {
            bloom_jolt_ffi_physics().body_get_rotation_axis(body, axis as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_position(
            body: f64,
            x: f64,
            y: f64,
            z: f64,
            ...
            bloom_jolt_ffi_physics().body_set_position(
                body,
                x as f32,
                y as f32,
                z as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_rotation(
            body: f64,
            x: f64,
            y: f64,
            z: f64,
            ...
            bloom_jolt_ffi_physics().body_set_rotation(
                body,
                x as f32,
                y as f32,
                z as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_transform(
            body: f64,
            px: f64,
            py: f64,
            pz: f64,
            ...
            bloom_jolt_ffi_physics().body_set_transform(
                body,
                px as f32,
                py as f32,
                pz as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_move_kinematic(
            body: f64,
            px: f64,
            py: f64,
            pz: f64,
            ...
            bloom_jolt_ffi_physics().body_move_kinematic(
                body, px as f32, py as f32, pz as f32, rx as f32, ry as f32, rz as f32, rw as f32,
                dt as f32,
            );
        }

        #[no_mangle]
        pub extern "C" fn bloom_physics_body_get_linear_velocity(body: f64, axis: f64) -> f64 {
            bloom_jolt_ffi_physics().body_get_linear_velocity_axis(body, axis as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_get_angular_velocity(body: f64, axis: f64) -> f64 {
            bloom_jolt_ffi_physics().body_get_angular_velocity_axis(body, axis as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_get_point_velocity(
            body: f64,
            px: f64,
            py: f64,
            pz: f64,
            ...
            bloom_jolt_ffi_physics().body_get_point_velocity_axis(
                body,
                px as f32,
                py as f32,
                pz as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_linear_velocity(
            body: f64,
            x: f64,
            y: f64,
            z: f64,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_angular_velocity(
            body: f64,
            x: f64,
            y: f64,
            z: f64,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_add_force(body: f64, x: f64, y: f64, z: f64) {
            bloom_jolt_ffi_physics().body_add_force(body, x as f32, y as f32, z as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_add_impulse(body: f64, x: f64, y: f64, z: f64) {
            bloom_jolt_ffi_physics().body_add_impulse(body, x as f32, y as f32, z as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_add_torque(body: f64, x: f64, y: f64, z: f64) {
            bloom_jolt_ffi_physics().body_add_torque(body, x as f32, y as f32, z as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_add_angular_impulse(
            body: f64,
            x: f64,
            y: f64,
            z: f64,
        ) {
            bloom_jolt_ffi_physics().body_add_angular_impulse(body, x as f32, y as f32, z as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_add_force_at(
            body: f64,
            fx: f64,
            fy: f64,
            fz: f64,
            ...
            bloom_jolt_ffi_physics().body_add_force_at(
                body, fx as f32, fy as f32, fz as f32, px as f32, py as f32, pz as f32,
            );
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_add_impulse_at(
            body: f64,
            ix: f64,
            iy: f64,
            iz: f64,
            ...
            bloom_jolt_ffi_physics().body_add_impulse_at(
                body, ix as f32, iy as f32, iz as f32, px as f32, py as f32, pz as f32,
            );
        }

        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_friction(body: f64, v: f64) {
            bloom_jolt_ffi_physics().body_set_friction(body, v as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_restitution(body: f64, v: f64) {
            bloom_jolt_ffi_physics().body_set_restitution(body, v as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_linear_damping(body: f64, v: f64) {
            bloom_jolt_ffi_physics().body_set_linear_damping(body, v as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_angular_damping(body: f64, v: f64) {
            bloom_jolt_ffi_physics().body_set_angular_damping(body, v as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_gravity_factor(body: f64, v: f64) {
            bloom_jolt_ffi_physics().body_set_gravity_factor(body, v as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_ccd(body: f64, enabled: f64) {
            bloom_jolt_ffi_physics().body_set_ccd(body, enabled != 0.0);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_motion_type(body: f64, t: f64, activate: f64) {
            bloom_jolt_ffi_physics().body_set_motion_type(body, t as u32, activate != 0.0);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_object_layer(body: f64, layer: f64) {
            bloom_jolt_ffi_physics().body_set_object_layer(body, layer as u32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_is_sensor(body: f64, enabled: f64) {
            bloom_jolt_ffi_physics().body_set_is_sensor(body, enabled != 0.0);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_allow_sleeping(body: f64, enabled: f64) {
            bloom_jolt_ffi_physics().body_set_allow_sleeping(body, enabled != 0.0);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_shape(
            body: f64,
            shape: f64,
            update_mass: f64,
            activate: f64,
        ) {
            bloom_jolt_ffi_physics().body_set_shape(
                body,
                shape,
                update_mass != 0.0,
                activate != 0.0,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_lock_rotation_axes(body: f64, x: f64, y: f64, z: f64) {
            bloom_jolt_ffi_physics().body_lock_rotation_axes(body, x != 0.0, y != 0.0, z != 0.0);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_lock_translation_axes(
            body: f64,
            x: f64,
            y: f64,
            z: f64,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_get_mass(body: f64) -> f64 {
            bloom_jolt_ffi_physics().body_get_mass(body) as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_get_friction(body: f64) -> f64 {
            bloom_jolt_ffi_physics().body_get_friction(body) as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_get_restitution(body: f64) -> f64 {
            bloom_jolt_ffi_physics().body_get_restitution(body) as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_get_object_layer(body: f64) -> f64 {
            bloom_jolt_ffi_physics().body_get_object_layer(body) as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_set_user_data(body: f64, lo: f64, hi: f64) {
            let user = (lo as u64) | ((hi as u64) << 32);
            bloom_jolt_ffi_physics().body_set_user_data(body, user);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_body_get_user_data(body: f64, part: f64) -> f64 {
        ...
        pub extern "C" fn bloom_physics_raycast(
            ...
            if bloom_jolt_ffi_physics().raycast_closest(
                world,
                ox as f32,
                oy as f32,
                oz as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_raycast_all(
            world: f64,
            ox: f64,
            oy: f64,
            oz: f64,
            ...
            bloom_jolt_ffi_physics().raycast_all(
                world,
                ox as f32,
                oy as f32,
                oz as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_ray_hit_count() -> f64 {
            bloom_jolt_ffi_physics().ray_hit_count() as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_ray_hit_body(i: f64) -> f64 {
            bloom_jolt_ffi_physics().ray_hit_body(i as usize)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_ray_hit_axis(i: f64, field: f64) -> f64 {
            bloom_jolt_ffi_physics().ray_hit_axis(i as usize, field as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_ray_hit_fraction(i: f64) -> f64 {
            bloom_jolt_ffi_physics().ray_hit_fraction(i as usize) as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_ray_hit_sub_shape(i: f64) -> f64 {
            bloom_jolt_ffi_physics().ray_hit_sub_shape(i as usize) as f64
        }

        #[no_mangle]
        pub extern "C" fn bloom_physics_overlap_sphere(
            world: f64,
            cx: f64,
            cy: f64,
            cz: f64,
            ...
            bloom_jolt_ffi_physics().overlap_sphere(
                world,
                cx as f32,
                cy as f32,
                cz as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_overlap_point(
            world: f64,
            px: f64,
            py: f64,
            pz: f64,
            ...
            bloom_jolt_ffi_physics().overlap_point(
                world,
                px as f32,
                py as f32,
                pz as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_overlap_box(
            world: f64,
            px: f64,
            py: f64,
            pz: f64,
            ...
            bloom_jolt_ffi_physics().overlap_box(
                world,
                px as f32,
                py as f32,
                pz as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_overlap_body(i: f64) -> f64 {
            bloom_jolt_ffi_physics().overlap_body(i as usize)
        }

        // --- Constraints ---

        #[no_mangle]
        pub extern "C" fn bloom_physics_constraint_fixed(
            body_a: f64,
            body_b: f64,
            ax: f64,
            ay: f64,
            ...
            bloom_jolt_ffi_physics().constraint_fixed(
                body_a,
                body_b,
                ax as f32,
                ay as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_constraint_point(
            body_a: f64,
            body_b: f64,
            ax: f64,
            ay: f64,
            ...
            bloom_jolt_ffi_physics().constraint_point(
                body_a,
                body_b,
                ax as f32,
                ay as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_constraint_hinge(
            body_a: f64,
            body_b: f64,
            ax: f64,
            ay: f64,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_constraint_slider(
            body_a: f64,
            body_b: f64,
            ax: f64,
            ay: f64,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_constraint_distance(
            body_a: f64,
            body_b: f64,
            ax: f64,
            ay: f64,
            ...
            bloom_jolt_ffi_physics().constraint_distance(
                body_a,
                body_b,
                ax as f32,
                ay as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_constraint_destroy(c: f64) {
            bloom_jolt_ffi_physics().constraint_destroy(c);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_constraint_set_enabled(c: f64, enabled: f64) {
            bloom_jolt_ffi_physics().constraint_set_enabled(c, enabled != 0.0);
        }

        // --- Contact events ---

        #[no_mangle]
        pub extern "C" fn bloom_physics_contact_count() -> f64 {
            bloom_jolt_ffi_physics().contact_count() as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_contact_field(i: f64, field: f64) -> f64 {
            bloom_jolt_ffi_physics().contact_field(i as usize, field as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_clear_contacts(world: f64) {
            let _ = world;
            bloom_jolt_ffi_physics().clear_contacts();
        }

        // --- Character controller (Tier 2) ---

        #[no_mangle]
        pub extern "C" fn bloom_physics_character_create(
            world: f64,
            shape: f64,
            up_x: f64,
            up_y: f64,
            up_z: f64,
            max_slope_angle: f64,
            character_padding: f64,
            penetration_recovery_speed: f64,
            predictive_contact_distance: f64,
            max_strength: f64,
            mass: f64,
            object_layer: f64,
            px: f64,
            ...
            bloom_jolt_ffi_physics().character_create(
                ...
                up_y as f32,
                up_z as f32,
                max_slope_angle as f32,
                character_padding as f32,
                penetration_recovery_speed as f32,
                predictive_contact_distance as f32,
                max_strength as f32,
                mass as f32,
                object_layer as u32,
                px as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_destroy(c: f64) {
            bloom_jolt_ffi_physics().character_destroy(c);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_update(
            c: f64,
            dt: f64,
            gx: f64,
            gy: f64,
            ...
            bloom_jolt_ffi_physics()
                .character_update(c, dt as f32, gx as f32, gy as f32, gz as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_get_position(c: f64, axis: f64) -> f64 {
            bloom_jolt_ffi_physics().character_get_position_axis(c, axis as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_get_rotation(c: f64, axis: f64) -> f64 {
            bloom_jolt_ffi_physics().character_get_rotation_axis(c, axis as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_set_position(c: f64, x: f64, y: f64, z: f64) {
            bloom_jolt_ffi_physics().character_set_position(c, x as f32, y as f32, z as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_set_rotation(
            c: f64,
            x: f64,
            y: f64,
            z: f64,
            ...
            bloom_jolt_ffi_physics()
                .character_set_rotation(c, x as f32, y as f32, z as f32, w as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_get_linear_velocity(c: f64, axis: f64) -> f64 {
            bloom_jolt_ffi_physics().character_get_linear_velocity_axis(c, axis as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_set_linear_velocity(
            c: f64,
            x: f64,
            y: f64,
            z: f64,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_get_ground_state(c: f64) -> f64 {
            bloom_jolt_ffi_physics().character_get_ground_state(c) as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_get_ground_normal(c: f64, axis: f64) -> f64 {
            bloom_jolt_ffi_physics().character_get_ground_normal_axis(c, axis as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_get_ground_position(c: f64, axis: f64) -> f64 {
            bloom_jolt_ffi_physics().character_get_ground_position_axis(c, axis as u32)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_get_ground_body(c: f64) -> f64 {
            bloom_jolt_ffi_physics().character_get_ground_body(c)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_character_set_shape(c: f64, shape: f64) {
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_soft_body_create(
            world: f64,
            vertex_count: f64,
            triangle_count: f64,
            px: f64,
            ...
            bloom_jolt_ffi_physics().soft_body_create_from_scratch(
                ...
                rw as f32,
                object_layer as u32,
                edge_compliance as f32,
                gravity_factor as f32,
                linear_damping as f32,
                pressure as f32,
            )
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_soft_body_vertex_count(body: f64) -> f64 {
            bloom_jolt_ffi_physics().soft_body_vertex_count(body) as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_soft_body_get_vertex(
            body: f64,
            idx: f64,
            axis: f64,
        ) -> f64 {
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_soft_body_set_vertex(
            body: f64,
            idx: f64,
            x: f64,
            y: f64,
            ...
            bloom_jolt_ffi_physics()
                .soft_body_set_vertex(body, idx as u32, x as f32, y as f32, z as f32);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_soft_body_set_vertex_inv_mass(
            body: f64,
            idx: f64,
            inv_mass: f64,
        ) {
            bloom_jolt_ffi_physics().soft_body_set_vertex_inv_mass(
                body,
                idx as u32,
                inv_mass as f32,
            );
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_vehicle_create(
            world: f64,
            chassis_shape: f64,
            up_x: f64,
            up_y: f64,
            ...
            max_handbrake_torque: f64,
            engine_max_torque: f64,
            max_pitch_roll_angle: f64,
            object_layer: f64,
            px: f64,
            py: f64,
            pz: f64,
            ...
            let (vh, _) = bloom_jolt_ffi_physics().vehicle_create(
                ...
                wheel_radius as f32,
                wheel_width as f32,
                suspension_min as f32,
                suspension_max as f32,
                max_steer_angle as f32,
                max_brake_torque as f32,
                max_handbrake_torque as f32,
                engine_max_torque as f32,
                max_pitch_roll_angle as f32,
                object_layer as u32,
                px as f32,
                py as f32,
                pz as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_vehicle_destroy(v: f64) {
            bloom_jolt_ffi_physics().vehicle_destroy(v);
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_vehicle_get_chassis(v: f64) -> f64 {
            bloom_jolt_ffi_physics().vehicle_get_chassis(v)
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_vehicle_set_input(
            v: f64,
            forward: f64,
            right: f64,
            brake: f64,
            ...
            bloom_jolt_ffi_physics().vehicle_set_input(
                v,
                forward as f32,
                right as f32,
                brake as f32,
        ...
        #[no_mangle]
        pub extern "C" fn bloom_physics_vehicle_get_wheel_transform(
            v: f64,
            wheel_index: f64,
            axis: f64,
        ) -> f64 {
            bloom_jolt_ffi_physics().vehicle_get_wheel_transform(v, wheel_index as u32, axis as u32)
                as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_vehicle_get_engine_rpm(v: f64) -> f64 {
            bloom_jolt_ffi_physics().vehicle_get_engine_rpm(v) as f64
        }
        #[no_mangle]
        pub extern "C" fn bloom_physics_vehicle_get_wheel_angular_velocity(
            v: f64,
            wheel_index: f64,
        ) -> f64 {
            bloom_jolt_ffi_physics().vehicle_get_wheel_angular_velocity(v, wheel_index as u32)
                as f64
        }
    };
}
...
mod step_tests {
    ...
    fn fixed_accumulator_matches_manual_stepping() {
        ...
        assert!(
            ...
            "expected ~half-step remainder, alpha = {alpha}"
        );
