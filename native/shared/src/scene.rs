...
use crate::renderer::{
    ...
    ImportedIridescenceDrawRef, ImportedRefractiveDrawRef, ImportedTransparentDrawRef,
    MeshDrawRef, Uniforms3D, Vertex3D,
};
...
impl SceneGraph {
    ...
    pub(crate) fn prepare_with_refraction(
        ...
        for (handle, node) in self.nodes.iter_mut() {
            ...
            if needs_secondary_uv && node.gpu_secondary_uv_vb.is_none() {
                node.gpu_secondary_uv_vb = node.secondary_tex_coords.as_ref().map(|coords| {
                    device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                ...
                && !node.indices.is_empty()
            {
                ...
                if node.gpu_geometry_key != Some(key) {
                    ...
                    let slice = if let Some(cached) = shared_gpu_geometry.get_mut(&key) {
                        ...
                        let slice =
                            gpu_driven.upload_static(device, queue, &node.vertices, &node.indices);
    ...
    pub(crate) fn has_visible_opaque_iridescence(
        &self,
        imported_refraction_enabled: bool,
    ) -> bool {
        self.nodes.iter().any(|(_handle, node)| {
...
fn scene_node_gpu_driven_ready(node: &SceneNode, imported_refraction_enabled: bool) -> bool {
    node.active_lod < 0
...
fn scene_geometry_key(vertices: &[Vertex3D], indices: &[u32]) -> SceneGeometryKey {
...
fn scene_material_key(material: &PbrMaterial) -> SceneMaterialKey {
    SceneMaterialKey {
        metal_rough: [
