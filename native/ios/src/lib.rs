...
unsafe extern "C" fn bloom_touches_began(
    _this: *mut c_void,
    _sel: Sel,
    touches: *const AnyObject,
    _event: *const AnyObject,
) {
    handle_touches(touches, TouchPhase::Began);
}

unsafe extern "C" fn bloom_touches_moved(
    _this: *mut c_void,
    _sel: Sel,
    touches: *const AnyObject,
    _event: *const AnyObject,
...
unsafe extern "C" fn bloom_touches_ended(
    _this: *mut c_void,
    _sel: Sel,
    touches: *const AnyObject,
    _event: *const AnyObject,
...
unsafe extern "C" fn bloom_touches_cancelled(
    _this: *mut c_void,
    _sel: Sel,
    touches: *const AnyObject,
    _event: *const AnyObject,
...
unsafe fn handle_touches(touches: *const AnyObject, phase: TouchPhase) {
    ...
    loop {
        ...
        let index = match phase {
            ...
            TouchPhase::Moved => match TOUCH_MAP.iter().position(|&p| p == touch_id) {
                Some(i) => i,
                None => continue,
            },
            TouchPhase::Ended => match TOUCH_MAP.iter().position(|&p| p == touch_id) {
                Some(i) => {
...
fn register_metal_view_class() {
    if AnyClass::get(c"BloomMetalView").is_some() {
        return;
    }
    ...
    unsafe {
        ...
        let cls = objc_allocateClassPair(
            superclass as *const AnyClass,
            b"BloomMetalView\0".as_ptr(),
            0,
        );
        if cls.is_null() {
            return;
        ...
        extern "C" {
            fn object_getClass(obj: *const c_void) -> *mut AnyClass;
        }
        ...
        class_addMethod(
            meta,
            sel!(layerClass),
            bloom_layer_class as *const c_void,
            b"#8@0:8\0".as_ptr(),
        );
        ...
        class_addMethod(
            ...
            bloom_touches_began as *const c_void,
            touch_types,
        );
        class_addMethod(
            cls,
            sel!(touchesMoved:withEvent:),
            bloom_touches_moved as *const c_void,
            touch_types,
        );
        class_addMethod(
            cls,
            sel!(touchesEnded:withEvent:),
            bloom_touches_ended as *const c_void,
            touch_types,
        );
        class_addMethod(
            cls,
            sel!(touchesCancelled:withEvent:),
            bloom_touches_cancelled as *const c_void,
            touch_types,
        );
...
fn register_view_controller_class() {
    if AnyClass::get(c"BloomViewController").is_some() {
        return;
    }
    ...
    unsafe {
        ...
        let cls = objc_allocateClassPair(
            superclass as *const AnyClass,
            b"BloomViewController\0".as_ptr(),
            0,
        );
        if cls.is_null() {
            return;
        ...
        class_addMethod(
            cls,
            sel,
            bloom_vc_supported_orientations as *const c_void,
            b"Q16@0:8\0".as_ptr(),
        );
...
unsafe extern "C" fn scene_will_connect(
    ...
    let handle = UiKitWindowHandle::new(std::ptr::NonNull::new(view_ptr).unwrap());
    ...
    {
        let ns_cls =
            AnyClass::get(c"NSTemporaryDirectory").unwrap_or(AnyClass::get(c"NSString").unwrap());
        // Use NSFileManager to get tmp dir
        extern "C" {
            fn NSTemporaryDirectory() -> *const AnyObject;
        }
        ...
        if !tmp_dir.is_null() {
            ...
            if !utf8.is_null() {
                ...
                if let Ok(tmp_path) = cstr_val.to_str() {
                    let _ = std::fs::write(
                        format!("{}bloom_debug.txt", tmp_path),
                        format!(
                            "scene_will_connect OK\npixels={}x{} scale={}\nwindow_scene={}\n",
                            pixel_width,
                            pixel_height,
                            scale,
                            !scene.is_null()
...
fn register_scene_delegate() {
    if AnyClass::get(c"PerrySceneDelegate").is_some() {
        return;
    }
...
pub extern "C" fn bloom_init_window(
    ...
    extern "C" {
        fn perry_ios_classes_registered();
    }
    unsafe {
    ...
    if unsafe { ENGINE.get().is_none() } {
        panic!(
            "[bloom-ios] Engine not initialized after 10s. \
