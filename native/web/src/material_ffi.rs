...
pub fn bloom_compile_material_refractive_str(source: &str) -> f64 {
    use bloom_shared::renderer::material_pipeline::{Bucket, FragmentProfile};
...
pub fn bloom_compile_material_instanced_str(source: &str) -> f64 {
    match engine().renderer.compile_material_instanced(source) {
        ...
        Err(e) => {
            web_sys::console::error_1(
                &format!("[material] instanced compile failed: {:?}", e).into(),
            );
            0.0
        }
    }
...
pub fn bloom_compile_material_from_file_str(path: &str, bucket_kind: f64) -> f64 {
    ...
    let (profile, bucket, reads_scene) = match bucket_kind as u32 {
        ...
        _ => {
            web_sys::console::error_1(
                &format!("[material] from_file: unknown bucket_kind {}", bucket_kind).into(),
            );
            return 0.0;
        }
    };
...
#[wasm_bindgen]
pub fn bloom_set_post_pass(_source: f64) -> f64 {
    0.0
}
...
pub fn bloom_set_post_pass_str(source: &str) -> f64 {
    match engine().renderer.set_post_pass(source) {
        Ok(()) => 1.0,
        Err(e) => {
            web_sys::console::error_1(&format!("[post_pass] compile failed: {:?}", e).into());
            0.0
        }
    }
}
...
#[wasm_bindgen]
pub fn bloom_add_post_pass(_source: f64) -> f64 {
    0.0
}
...
#[wasm_bindgen]
pub fn bloom_load_model_animation(_path: f64) -> f64 {
    0.0
}
...
pub fn bloom_update_model_animation(
    ...
    let eng = engine();
    eng.models
        .update_model_animation(handle, anim_index as usize, time as f32);
...
pub fn bloom_anim_finished(handle: f64) -> f64 {
    if engine().models.anim_finished(handle) {
        1.0
    } else {
        0.0
    }
...
pub fn bloom_particles_create(capacity: f64) -> f64 {
    ...
    let ib = eng
        .renderer
        .material_system
        .create_dynamic_instance_buffer(&eng.renderer.device, cap as u32);
...
pub fn bloom_particles_update(sys: f64, dt: f64) -> f64 {
    ...
    if live > 0 {
        ...
        eng.renderer
            .material_system
            .update_instance_buffer(&eng.renderer.queue, ib, &packed, live);
    }
    live as f64
}
...
pub fn bloom_set_reverb(size: f64, damp: f64, wet: f64) {
    engine()
        .audio
        .set_reverb(size as f32, damp as f32, wet as f32);
}

#[wasm_bindgen]
pub fn bloom_compile_material_instanced_bucket(
    source: String,
    bucket: f64,
    reads_scene: f64,
) -> f64 {
